...
function acquisitionQueries(jurisdiction: string, agency: string, officialDomains: string[] = []) {
  ...
  return [
    ...
    [`"${place}" ALPR`, "OTHER"],
    [`site:transparency.flocksafety.com "${place}"`, "STATISTICS"],
    ...
    [`site:.gov "${jurisdiction}" (Flock OR ALPR OR "license plate reader")`, "AGENCY"],
    ...
    ...officialDomains.flatMap((domain) => [
      [`site:${domain} (Flock OR ALPR OR "license plate reader")`, "OTHER"] as [string, EvidenceType],
      [`site:${domain} (agenda OR packet OR contract OR procurement OR policy)`, "AUTHORITY"] as [string, EvidenceType],
    ]),
...
export async function acquireJurisdictionAudit(frictionId: string, censusId?: string) {
  ...
  const frictionHit: SearchHit = { title: systematic ? "Systematic survey seed" : "Originating complaint", url: String(friction.origin_url), summary: String(friction.complaint_summary), evidenceType: systematic ? "OTHER" : "FRICTION" }; if (!systematic) await recordEvidence(auditId, frictionId, jurisdiction, frictionHit, "VERIFIED_COMPLAINT", frictionHit.summary);
